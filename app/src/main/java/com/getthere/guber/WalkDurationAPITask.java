 ...
 */
public class WalkDurationAPITask extends AsyncTask<Double, Void, Integer> {

    private Walk walk;

    public WalkDurationAPITask(Walk walk) { this.walk = walk; }

    @Override
    protected Integer doInBackground(Double... params) {
        double cur_lat = walk.start.latitude;
        double cur_long = walk.start.longitude;
        double dest_lat = walk.dest.latitude;
        double dest_long = walk.dest.longitude;
    ...
    @Override
    protected void onPostExecute(Integer duration) { walk.duration = duration; }
}